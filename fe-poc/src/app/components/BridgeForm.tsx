...
import { parseUnits, formatUnits, Address, Hex, pad, bytesToHex } from "viem";
...
} from "@solana/spl-token"; // Use sync version for simplicity here
...
import { toBigIntTokenAmount } from "../utils/utils";
...
const BridgeForm: React.FC = () => {
  ...
  const [sourceChain, setSourceChain] = useState<"solana" | "evm" | null>(
    "evm"
  );
  ...
  const isLoading =
    isQuotingFee || isBridging || isEvmSendPending || isApprovePending;
  ...
  const simulateEVMQuoteSend = async (sendParam: any) => {
    ...
    /*
        ...
        */
    console.warn(
      "EVM Quote simulation placeholder used. Implement with useSimulateContract or readContract."
    );
  ...
  const handleBridge = async (event: FormEvent) => {
    ...
    if (!quotedFee || !sourceChain || !amount || !recipientAddress) {
      setFeedback(
        "Error: Please quote the fee first and ensure all fields are filled."
      );
      setFeedbackStatus("error");
      return;
    }
    ...
    try {
      ...
      if (sourceChain === "evm") {
        ...
        try {
          ...
          const message =
            approvalError instanceof BaseError
      ...
      // --- Solana Bridge Send ---
        ...
        console.warn(
          "Solana transaction building requires proper Umi instruction to Web3.js conversion or manual construction."
        );
  ...
  async function BridgeTokensFromSolanaToEvm() {
    ...
    try {
      ...
      if (!walletClient) setFeedback("Please refresh the site");
      setFeedbackStatus("info");
      ...
      console.log(
        "🚀 ~ BridgeTokensFromSolanaToEvm ~recipientAddress:",
        recipientAddress
      );
      const recipientBytes32 = addressToBytes32(recipientAddress);

      console.log(
        "🚀 ~ BridgeTokensFromSolanaToEvm ~ recipientBytes32:",
        recipientBytes32
      );
      ...
      console.log(
        "🚀 ~ useDeposit ~ balanceBigInt < nativeFee:",
        balanceBigInt < nativeFee
      );
  ...
  const { data: evmTxReceipt, isLoading: isConfirming } =
    useWaitForTransactionReceipt({
      hash: evmSendTxHash,
      chainId: EVM_CHAIN_ID,
    });
